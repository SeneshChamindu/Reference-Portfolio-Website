# Consuming Prabath Kumara Design System in web apps

Read `artifacts/prabath-kumara/docs/AGENTS.md` first. This guide covers
...
`artifacts/prabath-kumara/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/prabath-kumara/styles.css";
```
...
`node_modules/@workspace/prabath-kumara/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/prabath-kumara/components/ui/button";
import { cn } from "@workspace/prabath-kumara/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/prabath-kumara/hooks/use-toast";
```
...
`@workspace/prabath-kumara/components/ui/button`. Run the app's typecheck
